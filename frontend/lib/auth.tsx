...
type User = { id?: string; email?: string; role?: string; companyId?: string } | null
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (token) {
      ...
      if (payload) setUser({ id: payload.sub, email: payload.email, role: payload.role, companyId: payload.companyId })
    }
  }, [])
  ...
  function setToken(token: string | null) {
    if (token) {
      ...
      const payload = decodeToken(token)
      if (payload) {
        const u: User = { id: payload.sub, email: payload.email, role: payload.role, companyId: payload.companyId }
