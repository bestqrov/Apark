...
export default function PermisCirculationForm() {
  ...
  async function handleSubmit(e: any) {
    ...
    try {
      const res = await fetch('/api/administratif/permis-circulation', { method: 'POST', body: fd })
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      <div className="bg-gradient-to-r from-indigo-600 to-indigo-700 text-white p-6 rounded-t-lg shadow-lg">
        ...
        <p className="text-indigo-100 text-sm mt-2">Gérez les permis de circulation des véhicules</p>
      </div>
        ...
        <div className="p-6 space-y-6">
          ...
          <FormSection title="Informations générales" icon="📋" color="green">
            <div>
              ...
              <input value={number} onChange={e => setNumber(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent" placeholder="PC-2026-001" />
            </div>
          </FormSection>

          <FormSection title="Montants" icon="💰" color="purple">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Montant HT (DH)</label>
                <input value={amountHt} onChange={e => setAmountHt(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-right focus:ring-2 focus:ring-purple-500 focus:border-transparent" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">TVA (%)</label>
                <input value={tva} onChange={e => setTva(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-right focus:ring-2 focus:ring-purple-500 focus:border-transparent" />
              </div>
            </div>
            <div className="mt-4 pt-4 border-t border-purple-200">
              <label className="block text-sm font-medium text-gray-700 mb-2">Montant TTC (DH)</label>
              <div className="px-3 py-3 bg-gradient-to-r from-purple-100 to-purple-50 border-2 border-purple-300 rounded-lg text-right font-bold text-lg text-purple-700">{montantTtc} DH</div>
            </div>
          </FormSection>

          <FormSection title="Période de validité" icon="📅" color="orange">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Date début</label>
                <input type="date" value={startDate} onChange={e => setStartDate(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Date fin</label>
                <input type="date" value={endDate} onChange={e => setEndDate(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent" />
              </div>
            </div>
          </FormSection>
