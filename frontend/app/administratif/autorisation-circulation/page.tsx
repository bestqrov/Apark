...
export default function AutorisationCirculationForm() {
  ...
  async function handleSubmit(e: any) {
    ...
    try {
      const res = await fetch('/api/administratif/autorisation-circulation', { method: 'POST', body: fd })
      if (!res.ok) throw new Error('Erreur')
      setMessage('Autorisation enregistrée')
    } catch (err) {
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      <div className="bg-gradient-to-r from-cyan-600 to-cyan-700 text-white p-6 rounded-t-lg shadow-lg">
        <h1 className="text-2xl font-bold flex items-center gap-3">
          <span className="text-3xl">✅</span>
          Autorisation de circulation
        </h1>
        <p className="text-cyan-100 text-sm mt-2">Gérez les autorisations de circulation</p>
      </div>
        ...
        <div className="p-6 space-y-6">
          ...
          <FormSection title="Informations générales" icon="📋" color="green">
            <div>
              ...
              <input value={number} onChange={e => setNumber(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent" placeholder="AC-2026-001" />
            </div>
          </FormSection>
          ...
          <FormSection title="Documents & Commentaires" icon="📎" color="orange">
            <div className="space-y-4">
              <div>
                ...
                <input type="file" onChange={e => setAttachment(e.target.files ? e.target.files[0] : null)} className="w-full text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-orange-50 file:text-orange-700 hover:file:bg-orange-100" />
              </div>
              <div>
