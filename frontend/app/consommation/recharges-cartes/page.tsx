"use client"
import React, { useState } from 'react'

export default function RechargesCartesPage() {
  const [card, setCard] = useState('')
  const [date, setDate] = useState('2025-12-31')
  const [amount, setAmount] = useState<number>(0)
  const [provider, setProvider] = useState('')
  const [comment, setComment] = useState('')
  const [status, setStatus] = useState<string | null>(null)

  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    setStatus('sending')
    try {
      const fd = new FormData()
      fd.append('card', card)
      fd.append('date', date)
      fd.append('amount', String(amount))
      fd.append('provider', provider)
      fd.append('comment', comment)
      const res = await fetch('/api/consommation/recharges-cartes', { method: 'POST', body: fd })
      if (!res.ok) throw new Error('err')
      setStatus('ok')
    } catch {
      setStatus('error')
    }
  }

  return (
    <div className="p-6">
      <h1 className="text-xl font-semibold mb-4">Recharge des cartes</h1>
      <form onSubmit={handleSubmit} className="space-y-4 max-w-2xl">
        <div className="grid grid-cols-2 gap-4">
          <label>
            <div className="text-sm text-slate-600">Carte</div>
            <input value={card} onChange={e => setCard(e.target.value)} className="mt-1 w-full p-2 border rounded" />
          </label>
          <label>
            <div className="text-sm text-slate-600">Date</div>
            <input type="date" value={date} onChange={e => setDate(e.target.value)} className="mt-1 w-full p-2 border rounded" />
          </label>
        </div>
        <div className="grid grid-cols-2 gap-4">
          <label>
            <div className="text-sm text-slate-600">Montant</div>
            <input type="number" step="0.01" value={amount} onChange={e => setAmount(Number(e.target.value))} className="mt-1 w-full p-2 border rounded" />
          </label>
          <label>
            <div className="text-sm text-slate-600">Fournisseur</div>
            <input value={provider} onChange={e => setProvider(e.target.value)} className="mt-1 w-full p-2 border rounded" />
          </label>
        </div>
        <label>
          <div className="text-sm text-slate-600">Commentaire</div>
          <textarea value={comment} onChange={e => setComment(e.target.value)} className="mt-1 w-full p-2 border rounded" />
        </label>
        <div className="flex gap-3">
          <button type="submit" className="px-4 py-2 bg-blue-600 text-white rounded">Enregistrer</button>
          {status === 'sending' && <div>Envoi...</div>}
          {status === 'ok' && <div className="text-green-600">OK</div>}
          {status === 'error' && <div className="text-red-600">Erreur</div>}
        </div>
      </form>
    </div>
  )
}
